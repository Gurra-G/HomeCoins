...
def GetChoreInfo(HomeId):
    ...
    Sql = """SELECT CHORE.chore_id, CHORE.chore_name, CHORE.chore_description, RESPONSIBILITY.chore_worth, 
             RESPONSIBILITY.user_id, user_name, deadline from CHORE join RESPONSIBILITY on 
...
def GetCompletedChores(HomeId):
    ...
    Cur = Conn.cursor()
    Sql = """SELECT CHORE.chore_id, CHORE.chore_name, CHORE.chore_description, RESPONSIBILITY.chore_worth, 
             RESPONSIBILITY.user_id, user_name, completed from CHORE join RESPONSIBILITY on 
             CHORE.chore_id = RESPONSIBILITY.chore_id
             join PERSON on RESPONSIBILITY.user_id = PERSON.user_id 
...
def UserInfos(HomeId):
    ...
    Conn = DataBaseConnect()
    Cur = Conn.cursor()
    Sql = """SELECT PERSON.user_id, PERSON.user_name, PERSON.user_email, PERSON.admin from PERSON
...
def ChoreRegistration(ChoreInfo):
    ...
    Conn = DataBaseConnect()
    Cur = Conn.cursor()
    Name, Description, Worth, UserId, Deadline = ChoreInfo
    ...
    Sql3 = """INSERT into RESPONSIBILITY(chore_id, user_id, 
                chore_worth, deadline) values(%s, %s, %s, %s)"""
    Cur.execute(Sql3, (ChoreId, UserId, Worth, Deadline))
    Conn.commit()
    DataBaseDisconnect()
...
def GetChores(UserId):
    ...
    Sql = """SELECT * from CHORE join RESPONSIBILITY on 
            CHORE.chore_id = RESPONSIBILITY.chore_id where user_id = %s;"""
    Cur.execute(Sql, (UserId,))
